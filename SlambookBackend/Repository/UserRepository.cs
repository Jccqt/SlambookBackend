...
namespace SlambookBackend.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<ServiceResponse<List<UserDTO>>> GetAllUsers(CancellationToken ct)
        {
            ...
            var users = await _db.Users
                ...
                .Select(u => new UserDTO
                {
                    ...
                    Username = u.Username,
                    Bio = u.Bio,
                    ProfilePicture = $"/api/user/profile/{u.Id}/profile-picture"
                }).ToListAsync(ct);
        ...
        public async Task<ServiceResponse<UserDTO>> GetUserById(int userId, CancellationToken ct)
        {
            ...
            if(user != null)
            {
                ...
                response.Data = new UserDTO
                {
                    ...
                    Bio = user.Bio,
                    ProfilePicture = $"/api/user/profile/{user.Id}/profile-picture"
                };
            }
            else
            {
        ...
        public async Task<ServiceResponse> UpdateLoginCount(int userId, CancellationToken ct)
        {
            ...
            if (affectedRows == 0)
            {
                response.Message = "Failed to update login count.";
            }
            else
            {
                response.Success = true;
